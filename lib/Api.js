...
const seasons = ["SEASON3", "SEASON2014", "SEASON2015", "SEASON2016"]
const default_season = "SEASON2016"

export default class Api{

  constructor(api_key, lazy) {
  ...
  getRegion(region) {
    if(region == undefined) region = this.default_region
  ...
  getSummonerByName(region, name) {
    ...
    // check if the user info is already in the local db
    return new Promise( (res, rej) => {
      this.lazy.queryOne("summoner", {name:name}, (item) => {
        if(item) {
          item["cached"] = true
          return res(item)
        }
        this.makeRequest(endpoint).then( (data) => {
          let name = Object.keys(data)[0]
          data["name"] = name
          this.lazy.insert("summoner", data)
          data["cached"] = false
          res(data)
        }, (err) => { rej(err) })
      })  
